#!/usr/bin/env python3
"""Batch review choros files against DS4 V4 Flash on G10.

Sends each file to the DS4 server with context about Choros constraints,
collects improvement suggestions, and writes them to /tmp/ds4_choros_review/.
"""
...
# Core files only (skip bench/experiment files)
FILES = [
    ("server.py", "Student-facing HTTP server"),
    ("workspace.py", "Filesystem persistence (MultiWorkspace, TeacherWorkspace, LessonStore)"),
    ("crystallize.py", "Single-shot crystallization engine"),
    ("crystallize_multipass.py", "Multi-pass crystallization (6 sequential calls)"),
    ("gen_lessons.py", "Lesson generation from crystallized curriculum"),
    ("teacher.js", "Teacher dashboard frontend JavaScript"),
    ("student.js", "Student lesson/quiz frontend JavaScript"),
    ...
    ("AGENTS.md", "Agent operating manual for coding agents"),
    ("DECISIONS.md", "Architecture decisions log"),
    ("MODELS.md", "Model tier strategy"),
    ("BASELINE.md", "Data source baseline and crystallization expectations"),
]
...
def review_file(filename, description):
    ...
    # Truncate to 6000 chars for speed (~13 tok/s, ~80s for 1024 tok response)
    if len(content) > 6000:
        content = content[:6000] + "\n\n[... truncated from {len(content)} chars]"
    
    prompt = f"""You are a senior software engineer reviewing the Choros project — an AI-powered curriculum mapping and lesson generation system for schools.
...
..."""
    ...
    try:
        ...
        if resp.returncode != 0:
            return f"[FAIL] {filename} — curl exited {resp.returncode} ({elapsed:.0f}s)"
        ...
        with open(out_path, "w") as f:
            ...
            f.write(f"*Response time: {elapsed:.0f}s, {len(text)} chars*\n\n")
            f.write(text)
...
print(f"Reviewing {len(FILES)} choros files against DS4 V4 Flash...\n")
results = []
...
print(f"\n{'='*60}")
print(f"Done. {len(os.listdir(OUT))} reviews in {OUT}/")
for r in results:
    print(f"  {r}")
